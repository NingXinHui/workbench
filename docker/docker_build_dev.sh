...
if [ -z "$cmd" ]; then
  ...
  echo "deploy_uat..."
  echo ${IMG_NAME}
  kubectl get secret dayaodevsecret
  if [ $? -ne 0 ]; then
    kubectl create secret docker-registry dayaodevsecret -n dayao-dev --docker-server=${ALI_REGISTRY_URL} --docker-username=${ALI_USER_NAME} --docker-password=${ALI_USER_PASSWORD}
  fi
  sed -e "s#{{SECRETS}}#dayaodevsecret#g;s#{{NAME_SPACE}}#${NAME_SPACE}#g;s#{{APP_NAME}}#$APP_NAME#g;s#{{APP_PORT}}#$APP_PORT#g;s#{{PROFILES}}#$PROFILES#g;s#{{APP_TARGET_PORT}}#$APP_TARGET_PORT#g;s#{{CONTAINER_PORT}}#$CONTAINER_PORT#g;s#{{IMAGE_URL}}#${ALI_REGISTRY_URL}/${ALI_HUANSI_YL_REGISTRY_NAME}/$APP_NAME#g;s#{{IMAGE_TAG}}#$IMAGE_TAG#g" docker/k8s_web.tpl > docker/k8s_web.yaml
