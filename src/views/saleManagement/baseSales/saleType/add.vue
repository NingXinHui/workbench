...
import {addSalesType,searchSalesType,updateSalesType} from "@/api/sale/base"
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      if (!checkFormData(this.formConfig.formItemList, this.queryForm)) {
       ...
       if(this.billCodeText == '新增') {
        addSalesType({
          ...this.queryForm,
          salesManList:this.queryForm.salesManList.toString(),
         ...
         updateSalesType({...this.queryForm,
         salesManList:this.queryForm.salesManList.toString(),
          tradeMethod:this.queryForm.tradeMethod.length == 1 ? this.queryForm.tradeMethod[0] : "0",
    ...
    initQueryForm(data = {}) {
      ...
      val.forEach(item => {
        if(this.queryForm.salesManList.indexOf(item.id) == -1) {
          this.queryForm.salesManList.push(item.id)
        }
      })
    },
  },
    ...
    formConfig() {
      ...
      let formItemList = [
        ...
        },
        {
          ...
          key: isEdit ? "salesManList" : "salesManList",
  ...
  mounted() {
    ...
    if(this.billCodeText == "编辑") {
      searchSalesType({id: this.detail.id}).then(res => {
        if(res.success) {
          this.initQueryForm({
            ...res.data,
            salesManList:res.data.salesManList.split(","),
            tradeMethod:res.data.tradeMethod == "0" ? ["1", "2"] : [res.data.tradeMethod]
          })
          this.$nextTick(() => {
            let arr = res.data.salesManList.split(",");
          let arr1 = res.data.salesManNameList.split(",");
