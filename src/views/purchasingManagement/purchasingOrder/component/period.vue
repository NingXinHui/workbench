...
-->
  <div>
    ...
    <div class="top_title">
          ...
          >
            <div style="display:flex;flex-warp:warp">
              ...
              <el-input
                ...
                placeholder=""  onkeyup="if(!/^[1-9][0-9]{0,10}?$/g.test(this.value)){this.value=''}"
              ></el-input>
              <span>日</span>
              <div class="line"></div>
              ...
              <div>
                <div v-for="(r, i) in item.accountPeriodDateDetailList" :key="i" :style="{'margin-top':i == 0 ? '0' : '10px'}">
              <div style="text-align:left">
                ...
                <el-input class="input" :disabled="disabled"  onkeyup="if(!/^[1-9][0-9]{0,10}?$/g.test(this.value)){this.value=''}" v-model="r.beginDay" placeholder=""></el-input>
                <span>日,生成应付,付款比例</span>
                <el-input class="input" :disabled="disabled" v-model="r.paymentRate"
                  onkeyup="if(!/^[0-9]+([0-9]{0,2})+(.[0-9]{0,2})?$/g.test(this.value)
                ){this.value=''}" @blur="r.paymentRate=='0' ? r.paymentRate=1 : ''"
      ...
      <div class="mid">
        <el-radio style="margin:9px 0 0 55px" :disabled="disabled" v-model="obj.saleAccountPeriodRule" :label="2"
          ><span class="tip">业务节点+分阶段</span></el-radio
        >
        <div class="detail" style="margin-left:180px">
          <div v-for="(r, i) in obj.periodDateList" :key="i" class="mt-10" v-show="r.type==2">
            <div>
              ...
              <el-input class="input"  onkeyup="if(!/^[1-9][0-9]{0,10}?$/g.test(this.value)){this.value=''}" :disabled="disabled" v-model="r.beginDay" placeholder=""></el-input>
              <span>日,生成应付,付款比例</span>
              <el-input class="input" :disabled="disabled" v-model="r.paymentRate"
               onkeyup="if(!/^[0-9]+([0-9]{0,2})+(.[0-9]{0,2})?$/g.test(this.value)
                ){this.value=''}" @blur="r.paymentRate=='0' ? r.paymentRate=1 : ''"
    ...
    if(this.selectRow.length) {
       if(this.selectRow[0].type == '1') {
        let arr = this.selectRow.map(item => {
          ...
          accountPeriodDateDetailList: item.accountPeriodDateDetailList.map(i => { return {...i,
