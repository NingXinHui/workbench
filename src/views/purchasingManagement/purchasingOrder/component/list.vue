...
<template>
    <div id="zPage">
      ...
      <div v-if="openmodal">
        <open-modal :billId="billId" billType="PURCHASE_ORDER" @closeModal="(value)=>{this.openmodal=false}" />
      </div>
    </div>
</template>
...
import {purchaseOrderList,deletePurchaseOrder,selectPurchaseType,submitPurchaseOrder,revokePurchaseOrder } from "@/api/purchase/order"
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        },
        {
          ...
          render: (h,scope) => {
            return h(
              'span',
              {},
              scope.row.purchaseDate.substring(0,10)
            )
          }
        },
        {
          ...
          render: (h,scope) => {
            return h(
              'span',
              {},
              scope.row.contractValidity.substring(0,10)
            )
          }
        },
        {
          prop: 'contractArchiving',
          align:"center",
          label: '合同是否归档',
          render:(h,scope) => {
            return h(
              ...
              {
                props: {
                  value: scope.row.contractArchiving,
                  disabled:true,
                }
              }
            )
        ...
        {
          prop: 'statisticalIdentificationName',
          label: '统计标识',
        },
        {
          prop: 'remark',
    ...
    searchArr() {
      return [
        ...
        },
        {
          ...
          optionValue: 'id',
          type: 2,
          selectOption: this.PurchaseType
        },
        {
          label: '创建时间',
          value: 'createDate',
  ...
  methods: {
    ...
    initParams() {
      let params = {
        ...
        purchaseTypeId:this.queryForm.purchaseTypeId
      };
    ...
    getDataList() {
      ...
      this.clearSelct()
      purchaseOrderList({...params,usable:true}).then(res => {
        this.loading = false
