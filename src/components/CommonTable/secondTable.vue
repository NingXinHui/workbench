<template>
 <div :class="!isSelectMore? 'table-list clearMoreCheck':'table-list' ">
    ...
    <el-table
              ...
              :row-class-name="(row)=>
          row.row.id===checkItemId?'setBackColorInFieldTable':''
        ">
      ...
      <el-table-column v-for="(items, index) in tableColumn"
                       header-align="center"
                       :key="index"
                       :prop="items.prop"
                       :label="items.label"
                       :type="items.type"
                       :width="items.width || (items.type === 'index' ? 50 : '')"
                       :fixed="items.fixed"
                       :class-name="items.className"
                       :show-overflow-tooltip="!isEdit"
                       :align="items.align ? items.align : 'left'">
        <el-table-column v-for="(item, index) in items.primary"
                       header-align="center"
                       :key="index"
                       :prop="item.prop"
                       :label="item.label"
        ...
        <template slot-scope="scope">
          <div v-if="scope.row.isEdit" class="editTable">
            ...
            <template v-else>
              <template v-if="item.editConfig">
                <el-checkbox @change="changeValue(scope,item)"
                ...
                <el-input v-if="item.editConfig.type==1"
                          :type="item.inputType?item.inputType:'text'"
                          :size="item.size?item.size:'medium'"
                          maxlength="40"
                          @focus="(item.openLook&&item.focus)?item.focus(scope):{}"
                          :disabled="!!item.disabled"
                          v-model="scope.row[item.prop]" />
